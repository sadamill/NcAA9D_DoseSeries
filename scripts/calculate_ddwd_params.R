# calculate gamma, beta, and b0

library(tidyverse)

source("scripts/dose_calculation.R")

scaling_params <- readr::read_csv(file = "input/raddose/scaling_params.csv") %>% 
  bind_cols(select(doses, fwd)[1:nrow(.),], .) %>% 
  mutate(k = 1/scale) %>% 
  mutate(
...
b_plot <- ggplot2::ggplot(scaling_params, aes(x = fwd, y = wilson_b, color = b_used)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(
...
g_plot <- ggplot2::ggplot(scaling_params, aes(x = fwd, y = k, color = k_used)) +
  ggplot2::geom_point() +
