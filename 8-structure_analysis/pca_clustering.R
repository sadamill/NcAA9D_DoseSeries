...
wide_wedges <- pseudohelixAtoms |> purrr::map(\(pdb) {
  pdb |> as_tibble() |> 
...
cluster <- factoextra::eclust(pca_filtered, FUNcluster = "kmeans", k = 3)
...
pca_filtered |> 
  bind_cols(number = 1:36) |> 
  ggplot(aes(x = number, y = PC1)) +
  geom_path()
  
pca$rotation |> as_tibble(rownames = "rowname") |> select(rowname:PC1) |> arrange(desc(abs(PC1))) |> view()

factoextra::fviz(pca, element = "ind")
