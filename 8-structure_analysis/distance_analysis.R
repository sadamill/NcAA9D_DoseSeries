...
stackedDistances <- purrr::map2(pseudohelixDose, pseudohelixList, \(dose, structure) {
...
multipleRegressions$Distances <- list(
  CuTyr = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-Tyr", weights = varIdent(form = ~ 1 | Molecule)), 
  CuNterm = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-NTerm", weights = varIdent(form = ~ 1 | Molecule)), 
  CuHis1ND1 = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-His1ND1", weights = varIdent(form = ~ 1 | Molecule)), 
  CuHis84NE2 = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-His84NE2", weights = varIdent(form = ~ 1 | Molecule)), 
  CuEq = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-Eq", weights = varIdent(form = ~ 1 | Molecule)), 
  CuAx = gls(Distance ~ Dose * Molecule, data = stackedDistances, subset = AtomPair == "Cu-Ax", weights = varIdent(form = ~ 1 | Molecule))
)

regressionSummaries$Distances <- dplyr::bind_rows(
  lapply(
    ...
    function(atom) {
      tibble::tibble(
        ...
        Coefficient = emtrends.coefficient(multipleRegressions$Distances[[atom]], "Dose"), 
        StandardError = emtrends.se(multipleRegressions$Distances[[atom]], "Dose"), 
        PValue = emtrends.pvalue(multipleRegressions$Distances[[atom]], "Dose")
      )
    }
  )
)

distance_scatter <- multipleRegressions$Distances |> 
  purrr::map(\(model) {
    data <- getData(model)
