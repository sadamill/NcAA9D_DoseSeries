...
for i in {1..36}
do
	mkdir -p $(printf "1-decay_parameter_estimation/output/wedge%02d" "$i")
done

for i in {1..25}
do
	mkdir -p $(printf "1-decay_parameter_estimation/output/start_frame_%02d/subwedges" "$i")
done
...
do
    (
		new_wedge_num=$((i-1))
	    cd $(printf "1-decay_parameter_estimation/output/wedge%02d" "$new_wedge_num")
...
do
    (
		cd $(printf "1-decay_parameter_estimation/output/start_frame_%02d" "$start_frame")
