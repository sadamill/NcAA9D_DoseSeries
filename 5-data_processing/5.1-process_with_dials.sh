...
for i in {1..36}
do
	mkdir -p $(printf "5-data_processing/input/" "")
done
...
do
    new_wedge_num=$((i-1))
    cd $(printf "wedge%02d" "$new_wedge_num")
    dials.import template=$(printf "../../input/Pk3Pn5/wedge%02d/HR00498_Pn5_wedge_%02d.####.gz" "$i" "$i")
...
do
    ...
    cd $(printf "pseudohelix%02d" "$pseudohelix_number")
